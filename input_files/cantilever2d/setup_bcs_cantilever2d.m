...
l = FE.mesh_input.box_dimensions(1);

tol = FE.max_elem_side/1000;

BR_pt  = find( ( FE.coords(1,:) > (l-tol)) & (FE.coords(2,:) > OPT.TR_min - tol) ...
    & (FE.coords(2,:) < OPT.TR_max + tol) );

L_edge = find(FE.coords(1,:) < tol);
%% ============================        
% Find columns that contain any value from idx
col_mask = any(ismember(FE.elem_node, L_edge), 1);
FE.BCs.fixed_indices = find(col_mask);
% ----------
col_mask = any(ismember(FE.elem_node, BR_pt), 1);
...
%% Applied forces
net_mag = -5;  % Force magnitude (net over all nodes where applied)
